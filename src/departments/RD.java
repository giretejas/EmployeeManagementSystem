...
public class RD {
	...
	public static void inserDataToDB(Employee emp) {
		try {
			...
			PreparedStatement stmt = con.prepareStatement("insert into empManagement.RD values (?,?,?,?,?,?,?,?,?)");
	...
	public static void deleteDataFromDB(int id) {
		try {
			PreparedStatement stmt = con.prepareStatement("delete from empManagement.RD where rid=?");
			stmt.setInt(1, id);
			stmt.executeUpdate();
	...
	public static void updateDataToDB(int id) {
		try {
			...
			if(dept.equals("RD") || dept.equals("rd")) {
				PreparedStatement stmt = con.prepareStatement("update empManagement.RD set empName=? ,dob=? ,doj=? ,doe=? ,dept=? ,managerName=? ,project=? ,salary=? where rid =?");
...
try {
			...
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.RD WHERE rid=?");
			stmt.setInt(1, id);
			...
			while(set.next()) {
				...
				System.out.println("********************************************************");
				System.out.println();
				System.out.println(colname.getColumnName(1)+" \t= \t"+set.getInt("rid"));
...
try {
			...
			PreparedStatement stmt2 = con.prepareStatement("select * from empManagement.RD where rid=?");
			stmt2.setInt(1, id);
	...
	public static void viewAllEmployee(String dept) {
		// TODO Auto-generated method stub
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.RD WHERE dept=?");
	...
	public static void viewOrgEmp() {
		// TODO Auto-generated method stub
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.RD");
