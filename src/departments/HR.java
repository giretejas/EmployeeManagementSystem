...
public class HR{
	...
	public static void inserDataToDB(Employee emp) {
		try {
			...
			PreparedStatement stmt = con.prepareStatement("insert into empManagement.HR values (?,?,?,?,?,?,?,?,?)");
	...
	public static void deleteDataFromDB(int id) {
		try {
			PreparedStatement stmt = con.prepareStatement("delete from empManagement.HR where hid=?");
			stmt.setInt(1, id);
			stmt.executeUpdate();
	...
	public static void updateDataToDB(int id) {
		try {
			...
			if(dept.equals("HR") || dept.equals("hr")) {
				PreparedStatement stmt = con.prepareStatement("update empManagement.HR set empName=? ,dob=? ,doj=? ,doe=? ,dept=? ,managerName=? ,project=? ,salary=? where hid =?");
...
try {
			...
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.HR WHERE hid=?");
			stmt.setInt(1, id);
			...
			while(set.next()) {
				...
				System.out.println("********************************************************");
				System.out.println();
				System.out.println(colname.getColumnName(1)+" \t= \t"+set.getInt("hid"));
...
try {
			...
			PreparedStatement stmt2 = con.prepareStatement("select * from empManagement.HR where hid=?");
			stmt2.setInt(1, id);
	...
	public static void viewAllEmployee(String dept) {
		// TODO Auto-generated method stub
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.HR WHERE dept=?");
	...
	public static void viewOrgEmp() {
		// TODO Auto-generated method stub
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.HR");
