...
public class Management {
	...
	public static void inserDataToDB(Employee emp) {
		try {
			...
			PreparedStatement stmt = con.prepareStatement("insert into empManagement.MANAGEMENT values (?,?,?,?,?,?,?,?,?)");
	...
	public static void deleteDataFromDB(int id) {
		try {
			PreparedStatement stmt = con.prepareStatement("delete from empManagement.MANAGEMENT where mid=?");
			stmt.setInt(1, id);
			stmt.executeUpdate();
	...
	public static void updateDataToDB(int id) {
		try {
			...
			if(dept.equals("MANAGEMENT") || dept.equals("management")) {
				PreparedStatement stmt = con.prepareStatement("update empManagement.MANAGEMENT set empName=? ,dob=? ,doj=? ,doe=? ,dept=? ,managerName=? ,project=? ,salary=? where mid =?");
...
try {
			...
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.MANAGEMENT WHERE mid=?");
			stmt.setInt(1, id);
			...
			while(set.next()) {
				...
				System.out.println("********************************************************");
				System.out.println();
				System.out.println(colname.getColumnName(1)+" \t= \t"+set.getInt("mid"));
	...
	public static void moveToOtherDb(int id) {
		...
		try {
			...
			PreparedStatement stmt2 = con.prepareStatement("select * from empManagement.MANAGEMENT where mid=?");
			stmt2.setInt(1, id);
	...
	public static void viewAllEmployee(String dept) {
		...
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.management WHERE dept=?");
	...
	public static void viewOrgEmp() {
		// TODO Auto-generated method stub
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.MANAGEMENT");
