...
public class Service {
	...
	public static void inserDataToDB(Employee emp) {
		try {
			...
			PreparedStatement stmt = con.prepareStatement("insert into empManagement.SERVICE values (?,?,?,?,?,?,?,?,?)");
	...
	public static void deleteDataFromDB(int id) {
		try {
			PreparedStatement stmt = con.prepareStatement("delete from empManagement.SERVICE where sid=?");
			stmt.setInt(1, id);
			stmt.executeUpdate();
	...
	public static void updateDataToDB(int id) {
		try {
			...
			if(dept.equals("SERVICE") || dept.equals("service")) {
				PreparedStatement stmt = con.prepareStatement("update empManagement.SERVICE set empName=? ,dob=? ,doj=? ,doe=? ,dept=? ,managerName=? ,project=? ,salary=? where sid =?");
...
try {
			...
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.SERVICE WHERE sid=?");
			stmt.setInt(1, id);
			...
			while(set.next()) {
				...
				System.out.println("********************************************************");
				System.out.println();
				System.out.println(colname.getColumnName(1)+" \t= \t"+set.getInt("sid"));
...
try {
			...
			PreparedStatement stmt2 = con.prepareStatement("select * from empManagement.service where sid=?");
			stmt2.setInt(1, id);
	...
	public static void viewAllEmployee(String dept) {
		// TODO Auto-generated method stub
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.service WHERE dept=?");
	...
	public static void viewOrgEmp() {
		// TODO Auto-generated method stub
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.service");
