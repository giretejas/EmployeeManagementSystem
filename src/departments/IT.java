...
public class IT {
	...
	public static void inserDataToDB(Employee emp) {
		try {
			...
			PreparedStatement stmt = con.prepareStatement("insert into empManagement.IT values (?,?,?,?,?,?,?,?,?)");
	...
	public static void deleteDataFromDB(int id) {
		try {
			...
			PreparedStatement stmt = con.prepareStatement("delete from empManagement.IT where iid=?");
			stmt.setInt(1, id);
			stmt.executeUpdate();
	...
	public static void moveToOtherDb(int id) {
		try {
			...
			PreparedStatement stmt2 = con.prepareStatement("select * from empManagement.IT where iid=?");
			stmt2.setInt(1, id);
	...
	public static void updateDataToDB(int id) {
		try {
			...
			if(dept.equals("IT") || dept.equals("it")) {
				PreparedStatement stmt = con.prepareStatement("update empManagement.IT set empName=? ,dob=? ,doj=? ,doe=? ,dept=? ,managerName=? ,project=? ,salary=? where iid =?");
	...
	public static void viewEmployee(int id) {
		try {
			...
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.IT WHERE iid=?");
			stmt.setInt(1, id);
			...
			while(set.next()) {
				...
				System.out.println("********************************************************");
				System.out.println();
				System.out.println(colname.getColumnName(1)+" \t= \t"+set.getInt("iid"));
	...
	public static void viewAllEmployee(String dept) {
		...
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.IT WHERE dept=?");
	...
	public static void viewOrgEmp() {
		// TODO Auto-generated method stub
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.IT");
