...
public class Operation {
	...
	public static void inserDataToDB(Employee emp) {
		try {
			...
			PreparedStatement stmt = con.prepareStatement("insert into empManagement.OPERATION values (?,?,?,?,?,?,?,?,?)");
	...
	public static void deleteDataFromDB(int id) {
		try {
			PreparedStatement stmt = con.prepareStatement("delete from empManagement.OPERATION where oid=?");
			stmt.setInt(1, id);
			stmt.executeUpdate();
	...
	public static void updateDataToDB(int id) {
		try {
			...
			if(dept.equals("OPERATION") || dept.equals("operation")) {
				PreparedStatement stmt = con.prepareStatement("update empManagement.OPERATION set empName=? ,dob=? ,doj=? ,doe=? ,dept=? ,managerName=? ,project=? ,salary=? where oid =?");
...
try {
			...
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.OPERATION WHERE oid=?");
			stmt.setInt(1, id);
			...
			while(set.next()) {
				...
				System.out.println("********************************************************");
				System.out.println();
				System.out.println(colname.getColumnName(1)+" \t= \t"+set.getInt("oid"));
...
try {
			...
			PreparedStatement stmt2 = con.prepareStatement("select * from empManagement.OPERATION where oid=?");
			stmt2.setInt(1, id);
	...
	public static void viewAllEmployee(String dept) {
		// TODO Auto-generated method stub
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.operation WHERE dept=?");
	...
	public static void viewOrgEmp() {
		// TODO Auto-generated method stub
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM empManagement.OPERATION");
